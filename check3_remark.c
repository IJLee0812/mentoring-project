...
int main(int argc, char *argv[]){ // argv[1] => 크기를 구하고 싶은 디렉토리의 경로 또는 이름
	init(argv[1]);
	if (strncmp("/", argv[1], 1) == 0) // 절대 경로로 입력했을 경우
		dfs(argv[1], "..");
	else if (strncmp("..", argv[1], 2) == 0) // ../dirname의 경우
		dfs(argv[1], "..");
	else if (strcmp(".", argv[1]) == 0) // . 입력했을 경우(프로그램 위치한 디렉터리) 탐색X, 출력
		dfs_print(0, ".");
	...
	while ((entry = readdir(dp)) != NULL){ // 현재 디렉터리 내용을 모두 읽었을 때 탈출
		...
		if (S_ISDIR(buf.st_mode)){ // 하위 디렉터리일 경우
			...
			if (strncmp(strrev(name), strrev(entry->d_name), strlen(entry->d_name)) == 0){ // <입력한 이름(경로) == 탐색한 디렉터리명>인 경우
				strrev(name); strrev(entry->d_name); // 비교를 위해 뒤집은 문자열 원상태로 복구
