...
void fileSize(int argc, char* argv[]){
	...
	inqueue(&Q,initNode(NULL,"",NULL));

	if((Q.front->dirp=opendir(absPath))==NULL){
		printf("Error : fail on open directory!\n");	
		exit(1);
	}
   	strcpy(Q.front->name,absPath);
	 ...
	 */
	while(Q.size){	
		if((dp=readdir(Q.front->dirp))!=NULL){		//다음 dp로 이동
			if(strcmp(dp->d_name,".")!=0&&strcmp(dp->d_name,"..")!=0){

			/*absPath = dp의 절대경로*/
            strcpy(absPath, Q.front->name);
            ...
            if((stbuf.st_mode&0xF000)==0x8000){		//dp가 file일 경우
               ...
               inqueue(&Q,initNode(opendir(absPath),absPath,NULL)); 
			}
